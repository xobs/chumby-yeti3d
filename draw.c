...
void CODE_IN_IWRAM draw_sprite(
  ...
  YETI_ROM sprite_t sprite, int u1, int v1, int u2, int v2,
...
#define AFFINE(I) {u16 c=tb[f2i(u+=uu)];if(c)fb[I]=c;}
          AFFINE_LOOP;
#undef  AFFINE
          }
          break;
...
#define AFFINE(I) {u16 c=tb[f2i(u+=uu)];if(c)fb[I]=RGB_BLEND(c,fb[I]);}
          AFFINE_LOOP;
#undef  AFFINE
          }
          break;
...
#define AFFINE(I) {fb[I] = RGB_BLEND(tb[f2i(u+=uu)], fb[I]);}
          AFFINE_LOOP;
#undef  AFFINE
          }
          break;
...
#define AFFINE(I) fb[I] = lua[f2i(l+=ll)][texture[f2i((v+=vv))&63][f2i((u+=uu))&63]];
      AFFINE_LOOP;
#undef  AFFINE
      }
      else
...
#define AFFINE(I) fb[I] = lut[texture[f2i(v+=vv)&63][f2i(u+=uu)&63]];
      AFFINE_LOOP;
#undef  AFFINE
      }
    }
